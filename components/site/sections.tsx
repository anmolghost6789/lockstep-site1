...
import { LifecycleScroll } from './lifecycle-scroll';
...
export function LifecycleSection() {
  ...
  const header = (
    <div className="flex flex-col gap-3">
      ...
      <h2 className="font-spectral text-[26px] leading-[1.15] tracking-[-1px] text-[#2d2f2e] dark:text-neutral-100 sm:text-[32px]">
        Six phases, each approved by the right person
      </h2>
      <p className="hidden max-w-[620px] font-inter text-[15px] leading-[1.6] text-[#646464] dark:text-neutral-400 sm:block">
        Scroll through a run. The agent writes each phase’s artifact, the right team approves it, and only then does the next phase start.
      </p>
    </div>
  );
  return (
    <section id="how" className="container relative scroll-mt-14 pb-10">
      <LifecycleScroll cards={PHASES} skills={skills.phases} common={skills.common} header={header} />
    </section>
  );
}
