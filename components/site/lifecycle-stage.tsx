...
import { ArrowUpRight, CheckCircle2, Clock3, FileText, Pause, Play } from 'lucide-react';
  ...
  return (
    <div ref={ref}>
      <LifecycleStage
        ...
        onBusy={setStageBusy}
      />
      <RunTrack cards={cards} stepOf={stepOf} started={started && !reduced} paused={paused} onTogglePause={() => setPaused((p) => !p)} onBusy={setGridBusy} />
      <PhaseGrid
        hideCards={compact}
        hideRail={compact}
        cards={cards}
...
/* -------------------------------------------------------------------------- */
/* Run track: the linear view under the stage (horizontal on desktop, vertical */
/* on phones), following the same run.                                         */
/* -------------------------------------------------------------------------- */
...
export const trackText = (card: PhaseCardData, step: RunStep) =>
  step === 'working' ? 'Agent working…' : step === 'waiting' ? `Waiting for ${card.role}` : step === 'approved' || step === 'idle' ? `Approved by ${card.role}` : `Gate: ${card.role}`;

export const trackChip = (step: RunStep) =>
  step === 'approved' || step === 'idle'
    ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400'
    : step === 'waiting'
      ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400'
      : step === 'working'
        ? 'bg-[#f9452d]/10 text-[#c2321e] dark:bg-[#E1F435]/10 dark:text-[#E1F435]'
        : 'bg-neutral-100 text-neutral-600 dark:bg-white/[0.05] dark:text-neutral-400';

export function TrackDot({ step }: { step: RunStep }) {
  const active = step === 'working' || step === 'waiting';
  const done = step === 'approved' || step === 'idle';
  return (
    <span
      aria-hidden
      className={cn(
        'relative z-10 grid size-[26px] shrink-0 place-items-center rounded-full border-2 bg-background transition-colors duration-300',
        active ? (step === 'waiting' ? 'border-amber-500' : 'border-[#f9452d] dark:border-[#E1F435]') : done ? 'border-emerald-500 bg-emerald-500' : 'border-neutral-300 dark:border-neutral-700',
      )}
    >
      {done && <CheckCircle2 className="size-4 text-white dark:text-neutral-950" strokeWidth={2.5} />}
      {active && (
        <>
          <span className={cn('size-2.5 rounded-full', step === 'waiting' ? 'bg-amber-500' : 'bg-[#f9452d] dark:bg-[#E1F435]')} />
          <span className={cn('absolute inset-0 animate-ping rounded-full opacity-40', step === 'waiting' ? 'bg-amber-400' : 'bg-[#f9452d] dark:bg-[#E1F435]')} />
        </>
      )}
    </span>
  );
...
function RunTrack({
  ...
  const steps = cards.map((_, i) => stepOf(i));
  const approved = steps.filter((s) => s === 'approved' || s === 'idle').length;
  const activeIdx = steps.findIndex((s) => s === 'working' || s === 'waiting');
  // Progress line fills up to the active stage (or all the way when the run is complete).
  const fill = activeIdx === -1 ? (approved === cards.length ? 1 : 0) : activeIdx / (cards.length - 1);

  return (
    <div className="mt-6" onPointerEnter={() => onBusy(true)} onPointerLeave={() => onBusy(false)}>
      <div className="mb-4 flex items-center justify-between gap-3">
        <p className="flex items-baseline gap-1.5 font-inter text-[13.5px] text-neutral-600 dark:text-neutral-400">
          <span className="font-spectral text-[24px] leading-none text-[#080808] dark:text-neutral-100">
            <NumberTicker value={approved} duration={0.4} />
          </span>
      ...
      {/* Desktop: horizontal */}
      <ol className="relative hidden grid-cols-6 gap-4 md:grid" aria-label="Lifecycle progress">
        <span aria-hidden className="absolute left-[13px] right-[calc(16.666%-13px)] top-[13px] h-0.5 rounded-full bg-neutral-200 dark:bg-neutral-800" />
        <span
          aria-hidden
          className="absolute left-[13px] top-[13px] h-0.5 rounded-full bg-emerald-500 transition-[width] duration-700 ease-out"
          style={{ width: `calc((100% - 16.666% ) * ${fill})` }}
        />
        {cards.map((card, i) => (
          <li key={card.id} className="relative">
            <TrackDot step={steps[i]} />
            <a
              href={`#skill-${card.id}`}
              className={cn(
                'mt-3 block rounded-xl border p-3 transition-[border-color,box-shadow] duration-300 hover:border-black/20 dark:hover:border-white/20',
                steps[i] === 'working' || steps[i] === 'waiting'
                  ? 'border-[#f9452d]/40 bg-white shadow-[0_10px_28px_-16px_rgba(249,69,45,0.5)] dark:border-[#E1F435]/40 dark:bg-[#0B0B0D]'
                  : 'border-black/[0.08] bg-white dark:border-white/[0.09] dark:bg-[#0B0B0D]',
              )}
            >
              <p className="font-mono text-[10px] uppercase tracking-[0.04em] text-neutral-400">{String(i + 1).padStart(2, '0')}</p>
              <p className="mt-0.5 font-inter text-[14px] font-semibold text-neutral-900 dark:text-neutral-100">{SHORT[card.id] ?? card.caption}</p>
              <p className="mt-0.5 truncate font-mono text-[11px] text-neutral-500">{card.file.replace('adlc/', '')}</p>
              <p className={cn('mt-2.5 flex items-center gap-1 rounded-md px-2 py-1 font-inter text-[11.5px] font-medium', trackChip(steps[i]))}>
                <TextStates text={trackText(card, steps[i])} className="truncate" />
              </p>
            </a>
          </li>
        ))}
      ...
      {/* Phones: vertical */}
      <ol className="relative space-y-3 pl-9 md:hidden" aria-label="Lifecycle progress">
        <span aria-hidden className="absolute bottom-4 left-[12px] top-4 w-0.5 rounded-full bg-neutral-200 dark:bg-neutral-800" />
        {cards.map((card, i) => (
          <li key={card.id} className="relative">
            <span className="absolute -left-9 top-3.5">
              <TrackDot step={steps[i]} />
            </span>
            <a
              href={`#skill-${card.id}`}
              className={cn(
                ...
                steps[i] === 'working' || steps[i] === 'waiting'
                  ? 'border-[#f9452d]/40 bg-white dark:border-[#E1F435]/40 dark:bg-[#0B0B0D]'
                  : 'border-black/[0.08] bg-white dark:border-white/[0.09] dark:bg-[#0B0B0D]',
              )}
            >
              <p className="font-inter text-[14.5px] font-semibold text-neutral-900 dark:text-neutral-100">
                <span className="mr-2 font-mono text-[11px] font-normal text-neutral-400">{String(i + 1).padStart(2, '0')}</span>
                {SHORT[card.id] ?? card.caption}
              </p>
              <p className="font-mono text-[11.5px] text-neutral-500">{card.file}</p>
              <p className={cn('mt-2 inline-flex rounded-md px-2 py-1 font-inter text-[12px] font-medium', trackChip(steps[i]))}>
                <TextStates text={trackText(card, steps[i])} />
              </p>
            </a>
          </li>
        ))}
