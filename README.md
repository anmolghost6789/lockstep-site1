...
The site is a single page: what it is (hero), how it works (a scroll-driven journey through the six phases),
the governance that runs with every phase (with the live audit log), the domain packs, pricing, the pilot offer,
and a short FAQ. Every navigation link points to a section of that
page; `/pricing` redirects to `#pricing` and `/packs` to `#packs`. The hero camera visits its cards in
lifecycle order (`STORY` in `app/home/HeroStage.tsx`). The main call to action and its address are set once in
`lib/site.ts`; replace the placeholder email before launch.
...
- **Packs and skills.** A pack is a whole package, such as a Claude Code project: its skills plus subagents,
  ...
  Five packs are preloaded: `adlc`, plus the data-engineering suite in `packs/`: `discovery-agent`, `requirements-agent`,
  `design-agent` and `build-agent`. New packs added to `packs/` (and to `SEED_PACKS` in `lib/registry/seed.ts`) are
  imported automatically, including into registries that already exist. Claude Code plugin layouts (`skills/` and
  `agents/` at the root, with `.claude-plugin/plugin.json`) are supported as well as `.claude/skills/`.
