...
# Runtime-owned projections that live under an otherwise user-owned input root.
# These files are derived from already-evaluated user preferences and consumed
# by renderers; treating them as fresh evidence creates a synthetic source and
# makes an unchanged extract look incremental. Natural user uploads elsewhere
# under ``context/branding`` remain part of the evidence corpus.
...
def _extract_pdf(path: Path, assets_dir: Path, asset_stem: str) -> tuple[str, list[str], str, str]:
    try:
        ...
        raise ImportError("install the declared 'pdfplumber' dependency to ingest PDFs") from exc
...
def ingest_run(run_dir: Path, roots: list[str], force: bool = False) -> dict:
    ...
    for root_name in roots:
        ...
        for file_path in _iter_files(root):
            if _rel(file_path, run_dir) in GENERATED_RUNTIME_PATHS:
                continue
