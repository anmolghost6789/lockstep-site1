...
inventory row, evidence ref, score) is dropped relative to the prior reports.
...
| **3 — Appendix** | Exhaustive inventories (all tables by layer, full source manifest, per-table scores) | Tables under a plain `## Appendix` heading, out of the decision path. |
...
> <1–2 sentence "why this verdict", no hedging>
```
...
- Line 3: at most two sentences of rationale.
...
> <1–3 sentence explanation. State the fact; cite specifics; no hedging.>
...
## Appendix                              ← full inventories (table-by-layer, source manifest)
```
...
`| Category | Files | Notes |` for the grouped view; the **full** per-file manifest
(`Source ID · Path · Type · Evidence role · Read status · Authority · Recency · Notes`)
goes in the Appendix. Conformance shown as a one-line token row above the table.
