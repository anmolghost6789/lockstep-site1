...
const COLUMNS: StageColumn[] = [
  {
    ...
    offset: 104,
    cards: [
      { id: '01 intent', node: <IntentCard /> },
      { id: 'lifecycle-activity', node: <LifecycleActivity /> },
    ],
  },
  {
    width: 330,
    offset: 0,
    cards: [
      { id: '02 level-1-plan', node: <PlanCard /> },
      { id: 'human-gate', node: <GateCard /> },
      { id: 'artifacts', node: <ArtifactNav /> },
    ],
  },
  {
    width: 378,
    offset: 56,
    cards: [
      { id: '03 bolts', node: <BoltsCard /> },
      { id: '04 code-review', node: <ReviewCard /> },
      { id: 'faq', node: <LifecycleFaq /> },
    ],
  },
];
...
// The camera follows the lifecycle in order, then loops: intent → plan → build → review → human gate →
// the artifacts it produced → the activity log → common questions → back to a new intent.
const STORY = ['01 intent', '02 level-1-plan', '03 bolts', '04 code-review', 'human-gate', 'artifacts', 'lifecycle-activity', 'faq'];
const ORDER = STORY.map((id) => CARDS.findIndex((card) => card.id === id)).filter((i) => i !== -1);
const START_INDEX = ORDER[0];
...
export function HeroStage({ className }: { className?: string }) {
  ...
  React.useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      // Next card in story order, wrapping back to the intent.
