...
export function validateSkill(
  ...
  if (rootSkill) {
    ...
    // lockstep-pack.json, or a Claude Code plugin manifest for plugin-style packs.
    const manifestFile = files.find((f) => f.path === 'lockstep-pack.json') ?? files.find((f) => f.path === '.claude-plugin/plugin.json');
